---
name: speckit-until-clean-remediate
...
metadata:
  author: Schroeder Studios
  source: until-clean:commands/remediate.md
---
...
`after_analyze` hook, so every `/speckit-analyze` continues into it, and it ends by invoking
`/speckit-analyze` again — whose hook invokes this again. The loop ends on a clean pass or on one of
...
- **The pass number**: count the `/speckit-analyze` reports in the current chain (since the maintainer's last
...
Invoke `/speckit-analyze`. Its `after_analyze` hook invokes this command again, which starts at step 1.