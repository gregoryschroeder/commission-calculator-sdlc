...
| VI. Accessible by default | PASS | Server-rendered semantic HTML per contracts/ui.md; structure asserted by web tests; colour contrast computed from the stylesheet's colour tokens by a unit test (tasks T031); keyboard-only check driven by key presses (T033); VoiceOver check by the maintainer (T034). A full axe scan in a browser is not planned; the checks above cover the WCAG criteria this page can fail (contrast, focus, labels, table headers, colour-only meaning). |
